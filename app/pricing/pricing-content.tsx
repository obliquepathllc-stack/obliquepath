...
const faqItems = [
  { question: "How do you determine the exact price for a project?", answer: "After an initial call where we map your operation and define scope, we provide a fixed quote. Price depends on complexity, number of integrations, and whether you need an automation suite or a full custom platform." },
  { question: "Can I upgrade or change the scope after we start?", answer: "Yes. If your needs change mid-build, we'll scope the additions, quote them separately, and integrate them into the existing system. Nothing gets added without your approval first." },
  { question: "Is there a setup fee?", answer: "No hidden fees. Setup, onboarding, and training are included in all engagements. What we quote is what you pay." },
  { question: "Do you offer discounts for non-profits or educational institutions?", answer: "Yes. We offer special pricing for non-profits, educational institutions, and early-stage startups. Get in touch with our team to discuss." },
  { question: "What happens if I need more than what we initially scoped?", answer: "We scope extensions separately and quote them as additions to your existing engagement. Your core system keeps running while additions are built and integrated." },
  { question: "How long does implementation typically take?", answer: "Simple automation builds typically go live in 4–6 weeks. Custom platforms run 4–5 months. We provide a specific timeline during scoping, before you commit." },
];
...
export function PricingContent() {
  return (
    <PageLayout title="Pricing" subtitle="We scope every build before quoting. No surprises, no hidden fees.">
        ...
        <div className="container max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {engagementTypes.map((type, i) => (
              <motion.div key={type.title} {...reveal(0.08 + i * 0.08)} className="rounded-2xl border border-border bg-card p-8 md:p-10 shadow-[0_2px_16px_-6px_rgba(0,0,0,0.07)] flex flex-col gap-6">
                <div>
                  <span className="text-[10px] tracking-[0.18em] uppercase font-medium text-primary border border-primary/20 bg-primary/5 rounded-full px-3 py-1">{type.tag}</span>
                </div>
                <div>
      ...
      <section className="px-4 lg:px-16 py-16 md:py-28">
        <div className="container max-w-6xl mx-auto">
          <motion.div {...reveal()} className="max-w-xl">
            ...
            <p className="text-muted-foreground mb-8 leading-relaxed">Book a free consultation. We&apos;ll map your operation, scope the build, and give you a clear answer on cost and timeline. No obligation.</p>
            <div className="flex flex-col sm:flex-row gap-3">
              ...
              <Link href="/contact">
                <button className="rounded-full border border-border text-foreground/70 font-medium px-7 py-3.5 hover:border-primary/40 hover:text-foreground transition-all duration-200">Contact Sales</button>
              </Link>
            </div>
          </motion.div>
        </div>
