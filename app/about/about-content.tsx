...
const values = [
  { title: "Accessibility", description: "Automation that's accessible to businesses of all sizes. Not just tech giants with large engineering budgets." },
  { title: "Practicality", description: "Solutions focused on real-world results and tangible improvements. Not demos that look good but never ship." },
  { title: "Accountability", description: "We're responsible for the build, the deployment, and the outcome. We don't disappear when it gets shipped." },
];
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24 items-start">
            ...
            <motion.div {...reveal(0.1)} className="grid grid-cols-2 gap-px bg-border">
              {impactStats.map((stat, i) => (
                <motion.div key={stat.label} {...reveal(0.08 + i * 0.05)} className="bg-background p-8 flex flex-col gap-2">
      ...
      <section className="px-4 lg:px-16 py-16 md:py-24 border-y border-border bg-secondary/30">
        <div className="container max-w-6xl mx-auto">
          ...
          <div className="flex flex-col">
            {values.map((v, i) => (
              <motion.div key={v.title} {...reveal(0.08 + i * 0.06)} className="grid grid-cols-1 md:grid-cols-[1fr_2fr] gap-6 md:gap-16 py-8 border-t border-border last:border-b">
        ...
        <div className="container max-w-6xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            <div>
              ...
              <div className="flex flex-col">
                {benefits.map((benefit, i) => (
                  <motion.div key={i} {...reveal(0.06 + i * 0.05)} className="py-4 border-t border-border last:border-b flex items-start gap-4">
                    <span className="text-xs font-mono text-muted-foreground/35 shrink-0 pt-0.5">{String(i + 1).padStart(2, "0")}</span>
            ...
            <motion.div {...reveal(0.12)} className="rounded-2xl border border-border bg-card p-10 shadow-[0_4px_32px_-8px_rgba(0,0,0,0.1)]">
              ...
              <p className="text-muted-foreground mb-8 leading-relaxed">30 minutes. No pitch. We&apos;ll tell you what to build, how long it takes, and what it costs.</p>
